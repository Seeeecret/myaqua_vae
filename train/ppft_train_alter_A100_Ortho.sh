#!/bin/bash
#
accelerate launch --mixed_precision="fp16" ppft_train_alter_A100_Ortho.py \
  ...
  --seed=2048 \
  --output_dir="/baai-cwm-1/baai_cwm_ml/public_data/scenes/lightwheelocc-v1.0/vae_data/Ortho_rank8_bits8_output_0329_valc" \
  --start_from_pretrain="8bit_state_dict_29.pth" \
  --validation_prompt="A portrait of a young white woman, masterpiece" \
  --validation_epochs=1 \
  --orthogonal_lambda=0.75 \
  --rank=8 \
  --OriginTaskLoraPath="/baai-cwm-1/baai_cwm_ml/public_data/scenes/lightwheelocc-v1.0/vae_data/juliensimon/stable-diffusion-v1-5-pokemon-lora/pytorch_lora_weights.safetensors" \
  --msg_bits=8