...
from safetensors.torch import save_file  # 确保导入save_file


def save_lora_weights_step(output_dir, unet, text_encoder, step, unet_keys):
    unet_lora_layers = unet_attn_processors_state_dict(unet, unet_keys)
    ...
    # 组合LoRA权重字典
    lora_state_dict = unet_lora_layers
    if text_encoder_lora_layers is not None:
        lora_state_dict.update(text_encoder_lora_layers)

    # 定义保存路径，包含步骤编号
    save_path = os.path.join(output_dir, f"./steploras/lora_weights_step_{step}.safetensors")
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help="Custom name prefix for saved LoRA models."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The path to the original task LoRA model.",
    )
    parser.add_argument(
        "--train_data_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help=("The lambda value for the orthogonal regularization."),
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0,
        help=("The lambda value for the l2 regularization."),
    )
...
def unet_attn_processors_state_dict(unet, unet_keys=None) -> Dict[str, torch.tensor]:
    r"""
    Returns:
...
def main(args):
    ...
    vae = AutoencoderKL.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae", ignore_mismatched_sizes=True, use_safetensors=True
    )
    ...
    unet_kets = None
    with open('/baai-cwm-1/baai_cwm_ml/algorithm/ziyang.yan/myaqua_vae/utils/unet_keys.json') as f:
        unet_keys = json.load(f)
    ...
    for key in unet_keys:
        ...
        if args.resume_from_lora is not None:
            lora.load_state_dict(
                {'down.weight': value_dict[key + '.down.weight'], 'up.weight': value_dict[key + '.up.weight']})
    ...
    if args.train_text_encoder:
        ...
        text_lora_parameters = CustomLoraLoaderMixin._modify_text_encoder(text_encoder, dtype=torch.float32,
                                                                          rank=args.rank, patch_mlp=True)
        ...
        if accelerator.is_main_process:
            ...
            for model in models:
                if isinstance(model, type(accelerator.unwrap_model(unet))):
                    unet_lora_layers_to_save = unet_attn_processors_state_dict(model, unet_keys)
    ...
    def load_model_hook(models, input_dir):
        ...
        while len(models) > 0:
            ...
            if isinstance(model, type(accelerator.unwrap_model(unet))):
                ...
                # 加载msgdecoder的状态字典
                msgdecoder_path = os.path.join(input_dir, "msgdecoder.pt")
                if os.path.exists(msgdecoder_path):
                    model.load_state_dict(torch.load(msgdecoder_path))
                    msgdecoder_ = model
                    logger.info(f"Loaded state for msgdecoder from {msgdecoder_path}")
                else:
                    logger.warning(f"msgdecoder state file {msgdecoder_path} not found.")
            elif isinstance(model, type(accelerator.unwrap_model(mapper))):
                # 加载mapper的状态字典
                mapper_path = os.path.join(input_dir, "mapper.pt")
                if os.path.exists(mapper_path):
                    model.load_state_dict(torch.load(mapper_path))
                    mapper_ = model
                    logger.info(f"Loaded state for mapper from {mapper_path}")
                else:
                    logger.warning(f"mapper state file {mapper_path} not found.")
            else:
                raise ValueError(f"unexpected save model: {model.__class__}")
    ...
    if args.dataset_name is not None:
        ...
        # 添加图像路径
        # dataset = dataset.map(lambda x: {"image": os.path.join(args.train_data_dir, x["file_name"])})
    ...
    accelerator.print("orthogonal_lambda:", args.orthogonal_lambda,"l2_lambda:", args.l2_lambda)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        l2_loss = 0.0
        # 超参数定义
        orthogonal_lambda = args.orthogonal_lambda  # 正交损失权重
        l2_lambda = args.l2_lambda  # L2正则化权重
        # 先前任务的lora
        matched_modules = load_file(args.OriginTaskLoraPath)
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                init_secret_vector = torch.randint(0, 2, (args.train_batch_size, args.msg_bits)).float().to(
                    accelerator.device)
                ...
                unet_lora_layers = unet_attn_processors_state_dict(accelerator.unwrap_model(unet), unet_keys)
                for name, param in unet_lora_layers.items():
                    # -------------------------------
                    # 1. 正交约束损失 (仅针对 lora_A)
                    # -------------------------------
                    if "lora.up" in name:
                        # 从预加载的 matched_modules 中获取对应参数（需提前加载）
                        if name in matched_modules:
                            param_ = matched_modules[name].to(param.device)  # 确保同设备
                            # 计算正交性约束: ||A * A_prev^T||_1
                            # orthogonal_loss += torch.abs(torch.mm(param, param_.T)).sum()
                            orthogonal_loss += torch.abs(torch.mm(param_.T, param)).sum()
                    # -------------------------------
                    # 2. L2正则化损失 (所有 lora_ 参数)
                    # -------------------------------
                    if "lora." in name:
                        l2_loss += torch.norm(param, p=2)  # L2范数

                total_loss = ppftloss + orthogonal_lambda * orthogonal_loss + l2_lambda * l2_loss

                # 将 total_loss 作为反向传播的损失
                loss = total_loss
                # Gather the losses across all processes for logging (if we use distributed training).
                avg_loss = accelerator.gather(loss.repeat(args.train_batch_size)).mean()
                train_loss += avg_loss.item() / args.gradient_accumulation_steps
                ppft_loss += ppftloss.item() / args.gradient_accumulation_steps
                orthogonal_loss = orthogonal_loss.item() / args.gradient_accumulation_steps
                l2_loss = l2_loss.item() / args.gradient_accumulation_steps
            ...
            logs = {"ppft_loss": ppftloss.item(), "orthogonal_loss": orthogonal_loss, "loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0],
                     "l2_loss": l2_loss}
            ...
            if accelerator.sync_gradients:
                ...
                accelerator.log({"orthogonal_loss": orthogonal_loss}, step=global_step)
                accelerator.log({"l2_loss": l2_loss}, step=global_step)
                ...
                if accelerator.is_main_process:
                    ...
                    if global_step >= args.max_train_steps - 10:
                        ...
                        # )
                        # logger.info(f"LoRA weights saved at step {global_step}")
                        # TODO: it works
                        unet_unwrapped = accelerator.unwrap_model(unet)
                        text_encoder_unwrapped = accelerator.unwrap_model(
                            text_encoder)
                        save_lora_weights_step(args.output_dir, unet_unwrapped, text_encoder_unwrapped, global_step,
                                               unet_keys)
                    ...
                    #         args.output_dir,
                    #         f"./steploras/{args.lora_save_name}_step{global_step}.safetensors"
                    #     )
                    #     CustomLoraLoaderMixin.save_lora_weights(
                    ...
                    #
                    #         text_encoder_unwrapped = accelerator.unwrap_model(text_encoder).to(
                    #             torch.float32) if args.train_text_encoder else None
                    #
                    #         save_lora_weights_step(args.output_dir, unet_unwrapped, text_encoder_unwrapped, global_step)
                    #     except Exception as e:
                    #         logger.error(f"Error saving LoRA weights at step {global_step}: {e}")
            # print every loss
