...
class Encoder(nn.Module):
    def __init__(self, latent_dim=256, input_length=135659520, kld_weight=0.005, kernel_size=5, stride=5, padding=1,
        ...
        self.fc_mu = nn.Linear(6956, self.latent_dim)
        # self.fc_logvar = nn.Linear(out_channels_list[-1] * self.final_feature_length, self.latent_dim)
        self.fc_logvar = nn.Linear(6956, self.latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, final_feature_length, latent_dim=256, kernel_size=5, stride=5, padding=1,
        ...
        if decoder_channel_list is None:
            self.decoder_channel_list = [4, 16, 64, 128, 256, 256, 64, 16, 8, 1]
        else:
...
class VAE(nn.Module):
    def __init__(self, latent_dim=256, input_length = 135659520, kld_weight=0.005,
        ...
        self.input_length = input_length
        if encoder_channel_list is None:
            encoder_channel_list = [4, 4, 4,  4, 4, 4, 4, 4]
        if decoder_channel_list is None:
            decoder_channel_list = [128, 64, 64, 32, 32, 16, 16, 4, 1]
        ...
        for i in range(len(decoder_channel_list) - 1):
            dim = real_input_dim // encoder_stride ** (len(decoder_channel_list) - i - 2)
    ...
    def adjust_input(self, input):
        ...
        input = torch.cat(
            [
                input,
                torch.zeros(input.shape[0], 1, (self.real_input_dim - self.input_length)).to(
                    input.device
                ),
            ],
            dim=2,
