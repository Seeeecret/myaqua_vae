...
class Encoder(nn.Module):
    def __init__(self, latent_dim=256, input_length=135659520, kld_weight=0.5, kernel_size=5, stride=4, padding=1,
        ...
        if encoder_channel_list is None:
            self.encoder_channel_list = [1, 2, 2, 4, 4, 8, 8, 16]  # 通道数列表
    ...
    def calculate_final_length(self, input_length):
        ...
        for _ in range(len(self.encoder_channel_list) - 1):
            length = (length + 2 * self.padding - self.kernel_size - 1) // self.stride + 1
        return length
...
class Decoder(nn.Module):
    def __init__(self, final_feature_length,latent_dim=256, kernel_size=5, stride=4, padding=1,
        ...
        if decoder_channel_list is None:
            self.decoder_channel_list = [16, 8, 8, 4, 4, 2, 2, 1]
        ...
        for idx, (in_c, out_c) in enumerate(zip(in_channels_list, out_channels_list)):
            layers.append(
                nn.ConvTranspose1d(
                    ...
                    stride=self.stride,
                    padding=self.padding,
                    output_padding=1  # 根据需要调整 output_padding 以匹配尺寸，之前长度不够，就是设置该值为1才使得长度貌似刚好匹配上了
                )
            )
            if out_c != 1:
                layers.append(nn.InstanceNorm1d(out_c))
...
class VAE(nn.Module):
    def __init__(self, latent_dim, input_length, kld_weight=0.5,
