...
class Encoder(nn.Module):
    def __init__(self, latent_dim=256, input_length=54263808, kld_weight=0.005, kernel_size=5, stride=4, padding=1,
        ...
        if encoder_channel_list is None:
            self.encoder_channel_list = [1, 2, 4, 8, 16, 32, 64, 128, 256]  # 通道数列表
        else:
...
class Decoder(nn.Module):
    def __init__(self, final_feature_length, latent_dim=256, kernel_size=5, stride=4, padding=1,
        ...
        if decoder_channel_list is None:
            self.decoder_channel_list = [256, 128, 64, 32, 16, 8, 4, 2, 1]
        else:
...
class VAE(nn.Module):
    def __init__(self, latent_dim, input_length=54263808, kld_weight=0.005,
