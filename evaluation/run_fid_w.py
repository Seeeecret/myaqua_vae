...
def main(args):
    ...
    # === 初始化水印模块 ===
    image_shield = ImageShield(
        ch_factor=1,
        hw_factor=4,
        height=args.image_length // 8,  # 潜在空间高度=原图高/8
        width=args.image_length // 8,  # 潜在空间宽度=原图宽/8
        device=device
    )
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        # 生成带水印的初始噪声
        watermarked_latents = image_shield.create_watermark().to(device=device, dtype=torch.float16)
