...
from WatermarkEvaluator_rotation import set_random_seed, WatermarkEvaluator
import pandas as pd
import csv  # 新增导入csv模块
    ...
    def run_robustness_experiment(self, distortion_types=None):
        ...
        with open(os.path.join(self.output_dir, 'clean_similarities.csv'), 'w', newline='') as clean_file, \
            ...
            for i in range(self.num_samples):
                ...
                _, score_clean, clean_similarities = self.compute_bit_acc(distorted_clean)
                _, score_wm, wm_similarities = self.compute_bit_acc(distorted_wm)
