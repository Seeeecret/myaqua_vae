...
from wartermark2_rotation import ImageShield  # 替换为实际模块路径
...
class WatermarkEvaluator:
    ...
    def compute_bit_acc(self, image):
        ...
        return self.shield.extract_watermark(inverted_latents)
    ...
    def run_experiment(self):
        ...
        # 创建CSV文件并写入表头
        with open(os.path.join(self.output_dir, 'clean_similarities.csv'), 'w', newline='') as clean_file, \
                open(os.path.join(self.output_dir, 'wm_similarities.csv'), 'w', newline='') as wm_file:

            clean_writer = csv.writer(clean_file)
            wm_writer = csv.writer(wm_file)

            # 写入表头
            clean_writer.writerow(['Similarity1', 'Similarity2', 'Similarity3', 'Similarity4'])
            wm_writer.writerow(['Similarity1', 'Similarity2', 'Similarity3', 'Similarity4'])
            ...
            for i in range(self.num_samples):
                ...
                _, score_clean, clean_similarities = self.compute_bit_acc(clean_img)
                scores.append(score_clean)
                labels.append(0)
                # 写入clean_similarities到CSV
                clean_writer.writerow(clean_similarities)
                pbar.update(1)

                # 计算加水印图像提取出的水印比特的准确率（正样本）
                _, score_wm, wm_similarities = self.compute_bit_acc(wm_img)
                scores.append(score_wm)
                labels.append(1)
                # 写入wm_similarities到CSV
                wm_writer.writerow(wm_similarities)
                pbar.update(1)
...
if __name__ == "__main__":
    evaluator = WatermarkEvaluator(
        ...
        dataset="/baai-cwm-nas/algorithm/ziyang.yan/wzyBackup/Stable-Diffusion-Prompts",
        model="/baai-cwm-nas/algorithm/ziyang.yan/wzyBackup/stable-diffusion-v1-5"
    )
