...
class OneDimVAEWatermark(nn.Module):
    def __init__(self, input_length, latent_dim=256, kernel_size=7, divide_slice_length=4096, kld_weight=0.02,
        ...
        # TODO: 修改模型结构
        d_model = [4, 8, 16, 16, 32, 32, 64, 64, 128, 256, 256, 512]
    ...
    def watermark(self, z):
        ...
        # 直接优化 z
        z_watermarked, retLoss, loss_i, loss_w = self._watermark_optim(z, n_iters=self.n_iters, lr=0.01)

        return z_watermarked, z, retLoss, loss_i, loss_w
    ...
    def encode_decode(self, input, **kwargs):
        ...
        # 添加水印
        z_watermarked, z_original, retLoss, loss_i, loss_w = self.watermark(z)
        ...
        return recons, input, mu, log_var, retLoss, loss_i, loss_w

    def _watermark_optim(self, z, n_iters=100, lr=0.01):
        # 创建 z_opt 并进行优化，但保持计算图连通
        z_opt = z.clone().detach().requires_grad_(True)  # 将 z 转换为叶子张量
        z_origin = z.clone().detach()
        ...
        for _ in range(n_iters):
            ...
            loss_i = F.mse_loss(z_opt, z_origin)  # 保持 z_opt 尽可能接近 z
            # 总损失 = 水印损失
            loss = self.lambda_w * loss_w + loss_i
        ...
        return z_final, retLoss, loss_i, loss_w

    def sample(self, enable_watermark=False, batch=1):
        z = torch.randn((batch, self.d_latent), device=self.device, dtype=torch.float32)
        if enable_watermark:
            z.requires_grad_(True)
            z_watermarked, z, retLoss, loss_i, loss_w = self.watermark(z)
            recons = self.decode(z_watermarked)
        else:
    ...
    def forward(self, x, **kwargs):
        recons, input, mu, log_var, retLoss, loss_i, loss_w = self.encode_decode(input=x, **kwargs)
        ...
        WatermarkLoss = retLoss

        return loss, recons_loss, kld_loss, WatermarkLoss, loss_i, loss_w
