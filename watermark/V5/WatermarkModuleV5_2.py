...
class WatermarkEncoder(nn.Module):
    ...
    def __init__(self, msg_length=56, latent_dim=256):
        super().__init__()
...
# 5_2更新: 将原先的图片VAE变成参数生成的VAE
class VAE_Watermark(nn.Module):
    """ 带水印嵌入的VAE主模型 """

    def __init__(self, input_length, latent_dim=256, kernel_size=7, divide_slice_length=1024, kld_weight=0.02,msg_length=32,
                 **kwargs):
        super().__init__()
        ...
        # 水印相关参数
        self.wm_encoder = WatermarkEncoder(msg_length=self.msg_length,latent_dim=latent_dim)
    ...
    def pad_sequence(self, input_seq, format_length=None):
        """
        ...
        if format_length is None:
            ...
            batch_size, channels, seq_length = input_seq.size()
            if seq_length < format_length:
                padding_size = self.adjusted_input_length - seq_length
                # 在最后一个维度上填充
                input_seq = F.pad(input_seq, (0, padding_size), "constant", 0)
            elif seq_length > format_length:
                # 截断多余的部分
                input_seq = input_seq[:, :self.adjusted_input_length]
        return input_seq
    ...
    def sample(self, batch=1):
        ...
        recons = self.pad_sequence(recons,format_length=self.initial_input_length)
        return recons
...
class WatermarkDecoder(nn.Module):
    ...
    def __init__(self, latent_dim=256, msg_length=56):
        super().__init__()
        self.model = nn.Sequential(
...
#     pad_x = model.pad_sequence(x)
#     pad_x = pad_x.squeeze(1)
#
#     # 计算ELBO损失
#     recon_loss = F.mse_loss(x_rec, pad_x, reduction='mean')
#     kl_loss = -0.5 * torch.sum(1 + logvar_wm - mu_wm.pow(2) - logvar_wm.exp())
#     loss = recon_loss + kl_loss
#
#     # 批量计算Hessian对角线
#     def compute_batch_hessian(z_params):
#         grad_first = grad(loss, z_params, create_graph=True)[0]
#         grad_second = []
#         for i in range(grad_first.shape[1]):
#             # 向量化计算每个维度的二阶导
#             grad_second_i = grad(grad_first[:, i].sum(), z_params, retain_graph=True)[0][:, i]
#             grad_second.append(grad_second_i)
#         return torch.stack(grad_second, dim=1).mean(dim=0)  # 按batch维度取平均
#
#     hessian_mu = compute_batch_hessian(mu_wm)
#     hessian_logvar = compute_batch_hessian(logvar_wm)
    ...
    #     hessian_mu.append(grad_i.mean().detach().to(model.device))  # 取均值作为期望估计
    #
    # # 计算logvar的Hessian对角线
    ...
    # return hessian_mu, hessian_logvar
def compute_diag_hessian(model, x, m):
    model.zero_grad()
    # 前向计算（强制使用FP32）
    with torch.cuda.amp.autocast(enabled=False):
        mu, logvar = model.encode(x.float())  # 确保输入为FP32
        mu_wm, logvar_wm = model.embed_watermark(mu, logvar, m)
        z = model.reparameterize(mu_wm, logvar_wm)
        x_rec = model.decode(z)
        pad_x = model.pad_sequence(x).squeeze(1).float()  # 确保目标为FP32
        # 计算损失
        recon_loss = F.mse_loss(x_rec, pad_x)
        kl_loss = -0.5 * torch.sum(1 + logvar_wm - mu_wm.pow(2) - logvar_wm.exp())
        loss = recon_loss + kl_loss
        # 计算一阶梯度（保留计算图）
        grad_mu = grad(loss, mu_wm, create_graph=True, retain_graph=True)[0]
        grad_logvar = grad(loss, logvar_wm, create_graph=True, retain_graph=True)[0]
        # 手动释放计算图
        loss = loss.detach()
        mu_wm = mu_wm.detach()
        logvar_wm = logvar_wm.detach()
        # 平方梯度近似Hessian对角线
        hessian_mu = grad_mu.pow(2).mean(dim=0)  # 按batch平均
        hessian_logvar = grad_logvar.pow(2).mean(dim=0)
    return hessian_mu, hessian_logvar

# ===================== 训练逻辑增强 =====================
class HessianAwareTrainer(nn.Module):
    def __init__(self, input_length, latent_dim=256, msg_length=56, beta=0.01):
        super().__init__()  #  关键修改2：调用父类初始化
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

        # 初始化模型
        self.vae = VAE_Watermark(latent_dim=latent_dim, input_length=input_length).to(self.device)
        ...
        # self.ema_gamma = self.ema_gamma.cuda()
        ...
        # 初始化EMA缓冲时直接放到设备上
        self.register_buffer('hessian_mu_ema', torch.ones(latent_dim, device=self.device))
        self.register_buffer('hessian_logvar_ema', torch.ones(latent_dim, device=self.device))
    ...
    def update_hessian_ema(self, x, m):
        """ 更新Hessian估计并调整敏感度系数 """
        # 计算当前batch的Hessian（启用梯度计算）
        with torch.enable_grad():  # 确保Hessian计算时梯度可追踪
            hessian_mu, hessian_logvar = compute_diag_hessian(self.vae, x, m)

        # 分离结果并转为CPU（如果需要）
        # hessian_mu = hessian_mu.detach()
        # hessian_logvar = hessian_logvar.detach()
        # print("logs: move begin")
        hessian_mu = hessian_mu.to(self.device)
        hessian_logvar = hessian_logvar.to(self.device)

        # print("logs: hessian_mu & hessian_logvar have moved to self.device")
    ...
    def train_step(self, x, m):
        ...
        # print(" VAE and watermark Train begin")
        # ===== VAE与水印联合训练 =====
        ...
        wm_loss = F.binary_cross_entropy_with_logits(wm_pred, m.float())
        ...
        robust_loss = F.binary_cross_entropy_with_logits(wm_pred_robust, m.float())
